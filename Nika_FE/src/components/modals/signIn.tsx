...
import { ADMIN_PATHS, API_URLS, MODAL_KEYS, ROLES } from '@/constants'

interface ISignInProps {
  isAdminPage?: boolean
}

const SignIn: React.FC<ISignInProps> = ({ isAdminPage = false }) => {
  ...
  const signUpSuccess = (role: string) => {
    closeModal()
    toast.success('Đăng nhập thành công !')
    if (isAdminPage && ROLES.admin === role) {
    ...
    <div className="w-96 max-sm:w-full pb-6">
      ...
      >
        ...
        <div className="flex gap-5 justify-center items-center mt-4">
          <button className="rounded-lg border primary-color px-5 py-1.5 leading-8 animation-btn">
            Đăng nhập
          </button>
        </div>
